...
const PanelProjectsList = () => {
  ...
  const handleDelete = async (id) => {
    const confirmDelete = await Swal.fire({
      ...
      text: 'Si borrás este proyecto se eliminará definitivamente.',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#db1a5a',
    ...
    if(confirmDelete.isConfirmed){
      try {
        ...
        const proyectosNuevos = proyectos.filter(proyecto => proyecto.id !== id);
        setProyectos(proyectosNuevos);
                    ...
                    {proyectos.map(proyecto => (
                      <li key={proyecto.id}>
                        ...
                        <div className="icons-wrapper">
                          ...
                          <Link className="delete-icon" onClick={() => handleDelete(proyecto.id)}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
